...
class MatmulBenchmark:
    ...
    def run_benchmark(self, M: int, N: int, K: int, dtype: torch.dtype):
        ...
        if dtype == torch.float32:
            ...
            try:
                ...
                print(
                    f"{time_basic:.3f} ms, {tflops_basic:.2f} TFLOPS, {speedup_basic:.2f}× vs PyTorch"
                )
        ...
        if dtype in [torch.bfloat16, torch.float16]:
            ...
            try:
                ...
                print(
                    f"{time_advanced:.3f} ms, {tflops_advanced:.2f} TFLOPS, {speedup_advanced:.2f}× vs PyTorch"
                )
        ...
        if results["triton_basic"]:
            ...
            print(
                f"  Triton Basic:    {results['triton_basic']['time_ms']:.3f} ms ({results['triton_basic']['tflops']:.2f} TFLOPS) [{pct:.1f}% of PyTorch]"
            )
        if results["triton_advanced"]:
            pct = (results["triton_advanced"]["tflops"] / tflops_pytorch) * 100
            print(
                f"  Triton Advanced: {results['triton_advanced']['time_ms']:.3f} ms ({results['triton_advanced']['tflops']:.2f} TFLOPS) [{pct:.1f}% of PyTorch]"
            )
        print(f"{'='*80}")
    ...
    def print_summary_table(self):
        ...
        print(f"{'='*80}")
        print(
            f"GPU: {self.gpu_name} ({self.gpu_memory_gb:.1f} GB, sm_{self.gpu_compute_capability})"
        )
        ...
        for shape, results in shapes.items():
            ...
            for r in results:
                speedup_str = (
                    f"{r.vs_pytorch_speedup:.2f}×" if r.vs_pytorch_speedup != 1.0 else "baseline"
                )
                print(
                    f"{r.implementation:<30} {r.time_ms:>10.3f}   {r.tflops:>8.2f}   {speedup_str:>10}"
                )
